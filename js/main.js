...
function dibuixa(){
	...
	for(var i=0; i<n1; i++){
		tmp.push({ id: m1+i, label: "1", shape: "ellipse", color: "#6E6EFD", x:-350, y:35*i })
	}

	// Balança 2
	for(var i=0; i<m2; i++){
		tmp.push({ id: i+n1+m1, label: "x", shape: "box", color: "#97C2FC", x:400, y:35*i})
	}
	for(var i=0; i<n2; i++){
		tmp.push({ id: m2+i+n1+m1, label: "1", shape: "ellipse", color: "#6E6EFD", x:450, y:35*i})
	}

	var multip = Math.max(n1, m1, n2, m2);
	console.log(multip);
	// Barra esquerra
	for(var i=0; i<8; i++){
